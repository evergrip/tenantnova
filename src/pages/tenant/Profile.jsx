...
export default function Profile() {
  ...
  async function save(e) { e.preventDefault(); const update = Object.fromEntries(allowed.map(k => [k, form[k] || ""])); const saved = await base44.entities.Tenant.update(access.tenant.id, update); await createAuditLog({ organizationId: access.organization_id, user: access.user, role: access.membership.role, action: "Tenant updated", entityType: "Tenant", entityId: access.tenant.id, beforeValues: access.tenant, afterValues: saved, reason: "Tenant updated own limited profile" }); }
